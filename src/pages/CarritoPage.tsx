...
export const CarritoPage = () => {
    const { listaCompras, aumentarCantidad, disminuirCantidad, eliminarCompra }: any = useContext(CarritoContext);

    const calcularTotal = () => {
        return listaCompras.reduce((total: number, item: { price: number; cantidad: number; }) => total + item.price * item.cantidad, 0).toFixed(2);
    }
    ...
    return (
        <>
            <table className="table">
                ...
                <tbody>
                    {
                        listaCompras.map((item: { id: string, title: string, price: number; cantidad: number; }) => (
                            <tr key={item.id}>
                                <th>{item.title}</th>
                                <td>{item.price}</td>
                                <td>
